...
/datum/wound/blunt/receive_damage(wounding_type, wounding_dmg, wound_bonus)
	if(!victim || (wounding_dmg < WOUND_MINIMUM_DAMAGE) || no_bleeding || (victim.blood_volume <= 0) || !prob(internal_bleeding_chance + wounding_dmg))
		return
	...
	if(limb.body_zone == BODY_ZONE_CHEST)
		...
		switch(blood_bled)
			...
			if(7 to 13)
				victim.visible_message(span_smalldanger("A thin stream of blood drips from [victim]'s mouth from the blow to [victim.p_their()] chest."), \
								span_danger("You cough up a bit of blood from the blow to your chest."), \
								vision_distance = COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled)
			if(14 to 19)
				victim.visible_message(span_smalldanger("Blood spews out of [victim]'s mouth from the blow to [victim.p_their()] chest!"), \
		...
		if(ishuman(victim) && (blood_bled >= 14))
			victim.AddComponent(/datum/component/creamed/blood)
