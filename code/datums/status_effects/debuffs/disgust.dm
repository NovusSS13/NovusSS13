...
/datum/status_effect/disgust/get_examine_text()
	...
	switch(disgust_value)
		if(DISGUST_LEVEL_GROSS to DISGUST_LEVEL_VERYGROSS)
			return span_warning("[owner.p_They()] look[owner.p_s()] a bit grossed out.")
		if(DISGUST_LEVEL_VERYGROSS to DISGUST_LEVEL_DISGUSTED)
			return span_warning("[owner.p_They()] look[owner.p_s()] really grossed out.")
		if(DISGUST_LEVEL_DISGUSTED to INFINITY)
			return span_warning("[owner.p_They()] look[owner.p_s()] extremely disgusted.")
...
/datum/status_effect/disgust/tick(seconds_per_tick, times_fired)
	...
	var/obj/item/organ/stomach/stomach = carbon_owner.get_organ_slot(ORGAN_SLOT_STOMACH_AID)
