...
//when you're in a liquid-filled turf
...
/datum/status_effect/water_affected/tick()
	...
	var/datum/reagents/turf_reagents = turf.liquids.simulate_reagents_flat(SUBMERGEMENT_REAGENTS_TOUCH_AMOUNT * SUBMERGEMENT_PERCENT(owner, turf.liquids))
	turf_reagents.expose(owner, TOUCH)
	qdel(turf_reagents)
	return ..()
