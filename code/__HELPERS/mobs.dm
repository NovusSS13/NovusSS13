...
/proc/random_features()
	...
	if(!length(GLOB.horns_list_lizard))
	...
	if(!length(GLOB.spines_list_lizard))
...
#define HAS_STASIS_FLAG(mob, flag) (mob.has_status_effect(/datum/status_effect/grouped/stasis)?:stasis_flags & flag)
...
/mob/proc/apply_pref_name(preference_type, client/requesting_client)
	...
	while(loop && safety < 5)
		if(!safety && !random)
			...
			newname = preference.create_default_value(requesting_client.prefs)
