...
/obj/item/organ/stomach/ethereal/on_insert(mob/living/carbon/stomach_owner)
	. = ..()
...
/obj/item/organ/stomach/ethereal/on_remove(mob/living/carbon/stomach_owner)
	. = ..()
	...
	if(do_after(carbon, 5 SECONDS, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_HELD_ITEM|IGNORE_INCAPACITATED)))
		if(ishuman(carbon))
			...
			if(human.dna?.species)
				...
				carbon.flash_lighting_fx(5, 7, tricolor_to_hex(human.dna.species.fixed_mut_color ? human.dna.species.fixed_mut_color : human.dna.features["mcolor"]))
