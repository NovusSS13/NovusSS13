...
/obj/item/organ/examine(mob/user)
	...
	. += span_info("[p_They()] should be inserted in the [parse_zone(zone)].")
	if(organ_flags & ORGAN_FAILING)
		if(IS_ROBOTIC_ORGAN(src))
			. += span_warning("[src] seem[p_s()] to be broken.")
			return
		. += span_warning("[src] [p_have()] decayed for too long, and [p_have()] turned a sickly color. [p_They()] probably won't work without repairs.")
