...
/atom/proc/balloon_alert_perform(mob/viewer, text)
	...
	balloon_alert.maptext = MAPTEXT("<span style='text-align: center; -dm-text-outline: 1px #0005'><span style='color: [maptext_color];'>[text]</span></span>")
	...
	var/content = span_balloontitle("<span style='color: [maptext_color];'>[src]</span>") + span_balloon("[capitalize(text)]")
	to_chat(viewer_client, balloon_block(span_info(content)))
