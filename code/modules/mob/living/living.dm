...
/mob/living/proc/ZImpactDamage(turf/T, levels)
	...
	visible_message(span_danger("[src] crashes into [T] with a sickening noise!"), span_userdanger("You crash into [T] with a sickening noise!"))
...
/mob/living/_pointed(atom/pointing_at)
	...
	visible_message("<span class='infoplain'>[span_name(span_color("[src]", chat_color))] points at [pointing_at].</span>", span_notice("You point at [pointing_at]."))
...
/mob/living/resist_grab(moving_resist)
	. = TRUE
	if(pulledby.grab_state || body_position == LYING_DOWN || HAS_TRAIT(src, TRAIT_CONSCIOUS_GRAB))
		var/altered_grab_state = pulledby.grab_state
		if((body_position == LYING_DOWN || HAS_TRAIT(src, TRAIT_CONSCIOUS_GRAB)) && pulledby.grab_state < GRAB_KILL) //If prone, resisting out of a grab is equivalent to 1 grab state higher. won't make the grab state exceed the normal max, however
			altered_grab_state++
...
/mob/living/throw_at(atom/target, range, speed, mob/thrower, spin = FALSE, diagonals_first = FALSE, datum/callback/callback, force, gentle = FALSE, quickstart = TRUE)
	stop_pulling()
	return ..()
...
/mob/living/proc/update_fire()
	var/datum/status_effect/fire_handler/fire_handler/fire_stacks/fire_stacks = has_status_effect(/datum/status_effect/fire_handler/fire_stacks)
	if(fire_stacks)
